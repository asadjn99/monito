...
// import { Package, Truck, Check, X, Clock, ChevronDown, ChevronUp, AlertCircle } from 'lucide-react';
// import Link from 'next/link';
// import Image from 'next/image';

// // --- COMPONENT: Order Timeline (Visual Progress) ---
// const OrderTimeline = ({ status }: { status: string }) => {
//   if (status === 'Cancelled' || status === 'Pending') return null;

//   const steps = [
//     { label: 'Placed', active: true, icon: <Clock size={14} /> },
//     { label: 'Accepted', active: ['Accepted', 'In-Progress', 'On the Way', 'Delivered'].includes(status), icon: <Check size={14} /> },
//     { label: 'On Way', active: ['On the Way', 'Delivered'].includes(status), icon: <Truck size={14} /> },
//     { label: 'Delivered', active: status === 'Delivered', icon: <Package size={14} /> },
//   ];
...
//              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm transition-all duration-500 border-2 shadow-sm ${step.active ? 'bg-[#003459] text-white border-[#003459]' : 'bg-white text-gray-300 border-gray-200'}`}>
//                 {step.icon}
//              </div>
//              <span className={`text-[10px] font-bold uppercase tracking-wider ${step.active ? 'text-[#003459]' : 'text-gray-300'}`}>{step.label}</span>
//          </div>
//        ))}
//     </div>
//   );
...
//   return (
//     <div className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${isOpen ? 'shadow-md border-blue-200' : 'shadow-sm border-gray-100 hover:border-blue-100'}`}>
...
//            <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl ${isOpen ? 'bg-[#003459] text-white' : 'bg-gray-100 text-gray-400'}`}>
//               <Package size={24} />
//            </div>
//            <div>
...
//               <p className="text-[10px] text-gray-400">{order.orderItems.length} Item(s)</p>
//            </div>
...
//                             {order.cancellationReason || "We apologize, but we could not fulfill your request at this time."}
//                         </p>
//                     </div>
//                 </div>
//             )}
...
//                     <div key={i} className="bg-white p-3 rounded-xl border border-gray-100 flex items-center gap-4 shadow-sm">
...
//                     <p><span className="font-bold">Payment:</span> {order.paymentInfo?.type || 'Bank Transfer'} <span className="text-gray-300">|</span> {order.shippingInfo?.city}</p>
//                     <p><span className="font-bold">Address:</span> {order.shippingInfo?.address}</p>
//                 </div>
//                 <div className="text-right w-full md:w-auto">
//                     <p className="text-xs uppercase font-bold tracking-widest text-gray-400">Total Paid</p>
//                     <p className="text-2xl font-extrabold text-[#003459]">{order.totalPrice.toLocaleString()} PKR</p>
//                 </div>
//             </div>
//         </div>
//       )}
...
//   if (!session) return (
//     <div className="min-h-screen flex items-center justify-center bg-[#FDF7E4]">
//         <div className="text-center">
//             <h2 className="text-2xl font-bold text-[#003459] mb-4">Access Restricted</h2>
//             <Link href="/login" className="px-6 py-2 bg-[#003459] text-white rounded-full font-bold">Login to View Orders</Link>
//         </div>
//     </div>
//   );
...
//        <div className="mb-10">
//           <h1 className="text-3xl font-extrabold text-[#003459]">Order History</h1>
...
//                  <Package className="text-4xl text-gray-300" />
//               </div>
//               <h3 className="text-lg font-bold text-gray-800">No Orders Yet</h3>
//               <p className="text-gray-500 mb-6">You haven't ordered any furry friends yet.</p>
//               <Link href="/" className="bg-[#003459] text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-800 transition">Start Shopping</Link>
//           </div>
//        ) : (
