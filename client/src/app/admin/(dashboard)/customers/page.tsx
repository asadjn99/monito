...
//   // --- FETCH REAL DATA ---
//   const fetchCustomers = async () => {
//     // setLoading(true); // Optional: Uncomment if you want spinner on every refresh
//     try {
//       // ⚠️ FIX: Added ?t=${Date.now()} to bust the cache
...
//     if (!confirm("Are you sure? This will delete the user and their data permanently.")) return;
...
//         // Update stats locally so count drops immediately
...
//                         <div className="flex items-center gap-3">
//                           <div className="w-10 h-10 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-bold text-lg overflow-hidden border-2 border-white shadow-sm">
//                              {user.image ? <img src={user.image} alt={user.name} className="w-full h-full object-cover"/> : (user.name?.charAt(0) || 'U').toUpperCase()}
//                           </div>
//                           <div>
//                              <p className="font-bold text-gray-900">{user.name || 'No Name'}</p>
//                              <p className="text-xs text-gray-500 font-mono">ID: {user.id.slice(-6)}</p>
...
// import React, { useState, useEffect } from 'react';
// import { 
//   Search, Users, Shield, UserPlus, Mail, Phone, Trash2, RefreshCw, Trophy
// } from 'lucide-react';
// import PetSpinner from '@/src/components/PetSpinner';
...
//   // --- FETCH REAL DATA ---
//   const fetchCustomers = async () => {
//     // Note: We don't set loading(true) here to avoid flickering on manual refresh
//     try {
//       // ⚠️ CACHE BUSTER: The '?t=' part forces a fresh load every time
//       const res = await fetch(`/api/admin/customers?t=${Date.now()}`, { 
//         cache: 'no-store',
...
//             className="flex items-center gap-2 text-sm font-bold text-blue-900 hover:bg-blue-50 px-3 py-2 rounded-lg transition"
//           >
...
//                       <td className="px-6 py-4">
//                         <div className="flex items-center gap-3">
//                           <div className="w-10 h-10 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-bold text-lg overflow-hidden border-2 border-white shadow-sm shrink-0">
...
//                  {topCustomers.length > 0 ? topCustomers.map((user, idx) => (
//                     <div key={user.id} className="flex items-center gap-3 pb-3 border-b border-gray-50 last:border-0 last:pb-0">
//                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shadow-sm text-white shrink-0
//                           ${idx === 0 ? 'bg-yellow-400 ring-2 ring-yellow-200' : idx === 1 ? 'bg-gray-400' : idx === 2 ? 'bg-orange-400' : 'bg-blue-900'}
//                        `}>
//                           {idx + 1}
//                        </div>
