...
//       try {
//         // ⚠️ CACHE BUSTER ADDED: ?t=${Date.now()} forces a fresh fetch
//         const [ordersRes, petsRes] = await Promise.all([
//           fetch(`/api/admin/orders?t=${Date.now()}`, { cache: 'no-store' }),
//           fetch(`/api/pets?t=${Date.now()}`, { cache: 'no-store' }) 
//         ]);
...
//           trend={stats.totalRevenue === 0 ? "No sales yet" : "+12.5%"} 
...
//           trend={stats.totalOrders === 0 ? "No orders yet" : "+5 new today"} 
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 🛑 FIX: Use 'total' (DB name) instead of 'totalPrice'
        // 🛑 FIX: Use 'status' (DB name) instead of 'orderStatus'
        const revenue = orders
          .filter((o: any) => o.status !== 'Cancelled')
          .reduce((acc: number, curr: any) => acc + (Number(curr.total) || 0), 0);

        // Count Pending Orders
        const pending = orders.filter((o: any) => o.status === 'New' || o.status === 'Pending').length;
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard 
          ...
          value={stats.totalOrders} 
          icon={<ShoppingBag size={24} />} 
          trend={stats.totalOrders === 0 ? "No orders yet" : "+ New"} 
          color="bg-blue-50 text-blue-700"
          iconBg="bg-white"
        />
        <StatCard 
              ...
              <tbody className="divide-y divide-gray-100">
                {stats.recentOrders.map((order: any) => (
                  <tr key={order.id} className="hover:bg-gray-50 transition-colors cursor-pointer group">
                    <td className="px-6 py-4 font-mono text-xs text-gray-500 font-bold">
                       {/* 🛑 FIX: Use 'orderId' (Readable) or 'id' */}
                       <Link href={`/admin/orders/${order.id}`}>
                         #{order.orderId || order.id?.slice(-6).toUpperCase()}
                       </Link>
                    </td>
                    <td className="px-6 py-4">
                      {/* 🛑 FIX: Check user.name OR customer (string) */}
                      <p className="font-bold text-gray-800 text-sm">
                        {order.user?.name || order.customer || 'Guest'}
                      </p>
                    </td>
                    <td className="px-6 py-4">
                      {/* 🛑 FIX: Pass 'status', not 'orderStatus' */}
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={`px-2 py-1 rounded-md text-[10px] font-extrabold uppercase ${badgeClass}`}>
      {status || 'Unknown'}
    </span>
  );
};